...
class DAC_Dataset(RNGDataFlow):
    ...
    def __init__(self, dataset_dir, train, all_classes):
        ...
        shuffle(self.images)
        if train == 0:
            self.images = self.images[0:len(self.images) // 10]
    ...
    def get_data(self):
        for image in self.images:
            ...
            # interpolation=cv2.INTER_LINEAR更适合于缩小
            #######################借PIL.image库实现无失真补正方形#############################
            # w, h = im.shape[0:2]
            # if w != h:
            #     im = my_image_op.make_square_cv(im)
            ################################################################################
            im = cv2.resize(im, (square_size, square_size), interpolation=cv2.INTER_LINEAR)
            ##################################灰度化的三通道##################################
            im_gray1 = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
            a = im_gray1.shape
            # im_gray3 = np.zeros(((square_size, square_size, 3)))
            # im_gray3 = np.asarray([im_gray1, im_gray1, im_gray1])
            im = im_gray1.reshape((square_size, square_size, 3))
            ################################################################################
...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        def halffire(name, x, num_squeeze_filters, num_expand_3x3_filters, skip):
            out_squeeze = Conv2D('squeeze_conv_' + name, x, out_channel=num_squeeze_filters, kernel_shape=1, stride=1,
                                 padding='SAME')
            out_squeeze = bn_activate('bn_squeeze_' + name, out_squeeze)
            out_expand_3x3 = Conv2D('expand_3x3_conv_' + name, out_squeeze, out_channel=num_expand_3x3_filters,
                                    kernel_shape=3, stride=1, padding='SAME')
...
def run_image(model, sess_init, image_dir):
    ...
    for image in images:
        ...
        im1 = cv2.resize(im, (square_size, square_size), interpolation=cv2.INTER_LINEAR)
        im1 = im1.reshape((1, square_size, square_size, 3))
        ...
        bndbox2 = {}
        bndbox2['xmin'] = int((bndboxes[0, max_h, max_w, 0] + max_w * down_sample_factor) * (IMAGE_WIDTH / square_size))
        bndbox2['ymin'] = int(
            (bndboxes[0, max_h, max_w, 1] + max_h * down_sample_factor) * (IMAGE_HEIGHT / square_size))
        bndbox2['xmax'] = int((bndboxes[0, max_h, max_w, 2] + max_w * down_sample_factor) * (IMAGE_WIDTH / square_size))
        bndbox2['ymax'] = int(
            (bndboxes[0, max_h, max_w, 3] + max_h * down_sample_factor) * (IMAGE_HEIGHT / square_size))
        ...
        # cv2.rectangle(im,
        #               (int(max_w * down_sample_factor * (IMAGE_WIDTH / square_size)),
        #                int(max_h * down_sample_factor * (IMAGE_HEIGHT / square_size))),
        #               (int((max_w + 1) * down_sample_factor * (IMAGE_WIDTH / square_size)),
        #                int((max_h + 1) * down_sample_factor * (IMAGE_HEIGHT / square_size))),
        #               (0, 255, 0), 4)
        # cv2.rectangle(im, (true_bndbox['xmin'], true_bndbox['ymin']), (true_bndbox['xmax'], true_bndbox['ymax']),
        #               (255, 0, 0), 4)
        cv2.rectangle(im, (bndbox2['xmin'], bndbox2['ymin']), (bndbox2['xmax'], bndbox2['ymax']), (0, 0, 255), 4)

        # cv2.imshow('image', im)
        cv2.imwrite('images_log/' + image, im)
        # cv2.waitKey(10)
    end = time()
    print("FPS:{}".format(len(images) / (end - start)))
...
def run_single_image(model, sess_init, image):
    ...
    if MONITOR == 1:
        monitor_names = ['conv_class_out', 'image_out', 'conv1_out', 'pool1_out', 'fire1_out', 'pool2_out', 'pool3_out',
                         'fire5_out', 'fire6_out', 'fire7_out']
    else:
        monitor_names = []
    output_names = ['objdetect_out', 'bndbox_out']
    ...
    if REAL_IMAGE == 1:
        ...
        # interpolation=cv2.INTER_LINEAR更适合于缩小
        ##################借PIL.image库实现在原图上无失真补正方形#############################
        w, h = im.size
        if w != h:
            im = PIL.Image.fromarray(cv2.cvtColor(im, cv2.COLOR_BGR2RGB))
            im = my_image_op.make_square(im)
...
def dump_weights(meta, model, output):
    ...
    with tf.Graph().as_default() as G:
        ...
        with sess.as_default():
            ...
            json_out, layers_list, max_cycles = generateLayers(sess, BITA, BITW, non_quantized_layers, target_frequency,
                                                               target_FMpS)
...
if __name__ == '__main__':
    ...
    if args.dump2_train1_test0 == '1':
        ...
        assert args.weights.endswith('.npy')
        run_single_image(Model(), DictRestore(np.load(args.weights, encoding='latin1', allow_pickle=True).item()),
                         args.run)
